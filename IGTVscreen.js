...
export default function IGTVscreen() {
  ...
  const Photoffl = ({ item }) => (
    <TouchableOpacity>
      <Image source={item.image} style={theme == 'light' ? styles.firstflimg : darkMode.IGTVFirstFlImg} />
    </TouchableOpacity>
  );

  const Photosfl = ({ item }) => (
    <TouchableOpacity>
      <Image source={item.image} style={theme == 'light' ? styles.secondflimg : darkMode.IGTVSecondFlImg} />
    </TouchableOpacity>
  );

  const typefl = ({ item }) => (
    <TouchableOpacity style={theme == 'light' ? styles.typebutton : darkMode.IGTVTypeButton}>
      <LinearGradient colors={item.gradient} style={theme == 'light' ? styles.backgroundbg : darkMode.IGTVBackgroundBg}>
        <Image source={item.image} style={theme == 'light' ? styles.typeflimage : darkMode.IGTVTypeFlImg} />
        <Text style={theme == 'light' ? styles.typetxt : darkMode.IGTVTypetxt}>{item.text}</Text>
      </LinearGradient>
    </TouchableOpacity>
  );
  ...
  return (
    ...
    <View style={theme == 'light' ? styles.container : darkMode.IGTVContainer}>
        <View style={theme == 'light' ? styles.images : darkMode.IGTVImages}>
          <View style={theme == 'light' ? styles.firstflcont : darkMode.IGTVFirstFlCont}>
            <FlatList
              scrollEnabled={false}
              style={theme == 'light' ? styles.firstfl : darkMode.IGTVFirstFl}
              data={firstfl}
              renderItem={Photoffl}
            />
          </View>
          <FlatList
            scrollEnabled={false}
            style={theme == 'light' ? styles.secondfl : darkMode.IGTVSecondFl}
            data={secondfl}
            renderItem={Photosfl}
          />
        </View>
